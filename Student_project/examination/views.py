...
def choose_skill(request):
    ...
    return render(request, 'examination/choose_skill.html', {'skills': skills})

def rules_and_regulations(request):
    return render(request, 'examination/rules_and_regulations.html')
...
def start_test(request, skill_id):
    ...
    if recent_test and recent_test.completed_date and (timezone.now() - recent_test.completed_date).days < 10:
        return render(request, 'examination/cannot_retake_test.html', {'next_attempt_date': recent_test.completed_date + timedelta(days=10)})
...
def take_test(request, test_id):
    ...
    return render(request, 'examination/take_test.html', {'test': test, 'questions': questions})


@login_required
def test_completed(request, test_id):
    test = get_object_or_404(Test, id=test_id, user=request.user)
    return render(request, 'examination/test_completed.html', {'test': test, 'home_url': '/'})