...
#         users = CustomUser.objects.filter(Q(email=email_or_phone) |Q(phone=email_or_phone))
#         if not users.exists():
...
#         print('data:' ,data)
#         if not CustomUser.objects.filter(email=data).exists() and not CustomUser.objects.filter(phone=data).exists():
#             raise forms.ValidationError("No user found with this email or phone.")
#         return data
...
#         # Debugging statement
#         print(f"clean_email_or_phone: {email_or_phone}")
