...
from .models import CustomUser, RequiterUser
...
class CandidateSignUpForm(UserCreationForm):
    ...
    def clean_email(self):
        ...
        if CustomUser.objects.filter(email=email).exists() or RequiterUser.objects.filter(email=email).exists():
            raise forms.ValidationError("A user with this email already exists.")
        return email

    def clean_phone(self):
        phone = self.cleaned_data.get('phone')
        if CustomUser.objects.filter(phone=phone).exists() or RequiterUser.objects.filter(phone=phone).exists():
